...
import demo.xmlassembly.omsxml.gen.privatecustomer.Collaterals;
import demo.xmlassembly.omsxml.gen.privatecustomer.Document;
import demo.xmlassembly.omsxml.gen.privatecustomer.Lifeinsurance;
...
@Component
public class PledgeLifeinsuranceXmlFactory {
    ...
    private void mapDataForDocumentType(Document document, PledgeLifeinsuranceConsolidatedData consolidatedData) {
        ...
        final List<Signee> signees = consolidatedData.getAllSignees();

        schemaPrivateCustomersDocumentUtils.setSignatures(document, signees);

        // -- now the more specific part

        document.getContent().setHeader( consolidatedData.getAddressee() );

        final Collaterals collaterals = new Collaterals();

        Lifeinsurance selectedLifeInsurancePolicy = new Lifeinsurance();
        selectedLifeInsurancePolicy.setPolicyNumber( consolidatedData.getMatchingLifeInsuranceCollateral().getPolicyNo());
        selectedLifeInsurancePolicy.setHolder( consolidatedData.getMatchingLifeInsuranceCollateral().getPolicyholderFullname());

        //let's make up these values on the spot
        selectedLifeInsurancePolicy.setPremium(BigDecimal.valueOf(1000));
        selectedLifeInsurancePolicy.setCoverageAmount(BigDecimal.valueOf(500_000));

        collaterals.getLifeinsurance().add( selectedLifeInsurancePolicy );
        document.getContent().setCollaterals( collaterals );

    } //m

    public String produceXml(PledgeLifeinsuranceConsolidatedData consolidatedData) {
